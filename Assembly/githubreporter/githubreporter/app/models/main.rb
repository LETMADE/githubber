class Main
  ...
  def self.push_pr_highlights(owner, repo_name, time_length, org_slug)
    ...
    pr_scores = Githubber.pr_scores(prs)

    prs_text.each do |pr|
      pr_score = pr_scores[pr['title']]
      self.send_highlight(pr, org_slug, pr_score)
    end
  end
  ...
  def self.send_highlight(text_package, org_slug)
    ...
    puts ""
    return Remote.highlight_post(url, text_package)
  end
  ...
  def self.push_user_highlights(owner, repo_name, time_length, org_slug)
    ...
    user_scores = Githubber.compute_user_scores(users)

    users.each do |u|
      m = Text.user_to_text(u, repo_name, user_scores)
  ...
  def self.push_commits_highlights(owner, repo_name, time_length, top_n, org_slug, data)
    ...
    commit_scores = Githubber.commit_scores(commits)

    commits.each do |commit|
      commit_score = commit_scores[commit['sha']]
      commit_text = Text.commit_to_text(commit, owner, repo_name, commit_score)
  ...
  def self.push_all(owner, repo_name, time_length_days, org_slug)
    ...
    self.push_commits_highlights(owner, repo_name, time_length, 30, org_slug, data)
  end
end
