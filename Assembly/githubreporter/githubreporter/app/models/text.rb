class Text
  ...
  def self.text_from_highlights(highlights, owner, repo_name)
    t = {}

    t['users'] = highlights['users'].map{|a| self.user_to_text(a, repo_name)}
    t['prs'] = highlights['prs'].map{|a| self.pr_to_text(a)}
    t['files'] = highlights['files'].map{|a| self.file_to_text(a, owner, repo_name)}
